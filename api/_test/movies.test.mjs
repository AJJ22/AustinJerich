import { test } from 'node:test'
import assert from 'node:assert'
import express from 'express'
import request from 'supertest'
import { createRouter } from '../_routes/movies.mjs'
import { MockMovieRepository, sampleMovies } from './mocks/movieRepository.mock.mjs'

// Helper function to create Express app with mocked repository
function createTestApp(mockRepository) {
  const app = express()
  app.use(express.json())
  app.use('/movies', createRouter(mockRepository))
  return app
}

test('GET /movies - Returns all movies', async (t) => {
  await t.test('should return array of movies', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).get('/movies')

    assert.equal(res.status, 200)
    assert.ok(Array.isArray(res.body))
    assert.equal(res.body.length, 3)
  })

  await t.test('should include movie details', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).get('/movies')

    assert.equal(res.body[0].title, 'The Understory')
    assert.equal(res.body[0].director, 'James W. Tolan')
  })

  await t.test('should call repository getAll method', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    await request(app).get('/movies')

    const calls = mockRepo.getCallHistory('getAll')
    assert.equal(calls.length, 1)
  })
})

test('GET /movies/:id - Returns single movie', async (t) => {
  await t.test('should return movie by ID', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).get('/movies/69263e138d6f9ef025a3be7f')

    assert.equal(res.status, 200)
    assert.equal(res.body.title, 'The Understory')
  })

  await t.test('should return 400 for invalid id', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).get('/movies/nonexistent')

    assert.equal(res.status, 400)
    assert.equal(res.body.message, 'invalid id')
  })

  await t.test('should return 400 for invalid ID format', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).get('/movies/invalid-id-format')

    assert.equal(res.status, 400)
    assert.equal(res.body.message, 'invalid id')
  })

  await t.test('should call repository getById method', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    await request(app).get('/movies/69263e138d6f9ef025a3be7f')

    const calls = mockRepo.getCallHistory('getById')
    assert.equal(calls.length, 1)
  })
})

test('POST /movies - Create new movie', async (t) => {
  await t.test('should create a new movie', async () => {
    const mockRepo = new MockMovieRepository([])
    const app = createTestApp(mockRepo)
    const newMovie = { title: 'Test Movie', director: 'Test Director', status: 'read', rating: 5 }

    const res = await request(app).post('/movies').send(newMovie)

    assert.equal(res.status, 200)
    assert.ok(res.body.insertedId)
    assert.equal(res.body.acknowledged, true)
  })

  await t.test('should reject array input with 400 status', async () => {
    const mockRepo = new MockMovieRepository([])
    const app = createTestApp(mockRepo)

    const res = await request(app).post('/movies').send([{ title: 'Movie 1' }, { title: 'Movie 2' }])

    assert.equal(res.status, 400)
    assert.equal(res.body.message, 'array not allowed')
  })

  await t.test('should call repository create method', async () => {
    const mockRepo = new MockMovieRepository([])
    const app = createTestApp(mockRepo)
    const newMovie = { title: 'Test Movie' }

    await request(app).post('/movies').send(newMovie)

    const calls = mockRepo.getCallHistory('create')
    assert.equal(calls.length, 1)
  })

  await t.test('should store data in repository', async () => {
    const mockRepo = new MockMovieRepository([])
    const app = createTestApp(mockRepo)
    const newMovie = { title: 'New Movie', director: 'New Director' }

    await request(app).post('/movies').send(newMovie)

    assert.equal(mockRepo.movies.length, 1)
    assert.equal(mockRepo.movies[0].title, 'New Movie')
  })
})

test('PUT /movies/:id - Update movie', async (t) => {
  await t.test('should update existing movie', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)
    const updates = { title: 'Updated Title', rating: 5 }

    const res = await request(app).put('/movies/69263e138d6f9ef025a3be7f').send(updates)

    assert.equal(res.status, 200)
    assert.equal(res.body.title, 'Updated Title')
    assert.equal(res.body.rating, 5)
  })

  await t.test('should return 400 for invalid id', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).put('/movies/nonexistent').send({ title: 'New Title' })

    assert.equal(res.status, 400)
  })

  await t.test('should return 400 for invalid ID format', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).put('/movies/invalid-id').send({ title: 'New Title' })

    assert.equal(res.status, 400)
  })

  await t.test('should call repository update method', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    await request(app).put('/movies/69263e138d6f9ef025a3be7f').send({ title: 'Updated' })

    const calls = mockRepo.getCallHistory('update')
    assert.equal(calls.length, 1)
  })

  await t.test('should trim string values', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).put('/movies/69263e138d6f9ef025a3be7f').send({ title: '  Trimmed Title  ' })

    assert.equal(res.body.title, 'Trimmed Title')
  })
})

test('DELETE /movies/:id - Delete movie', async (t) => {
  await t.test('should delete existing movie', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).delete('/movies/69263e138d6f9ef025a3be7f')

    assert.equal(res.status, 204)
    assert.equal(mockRepo.movies.length, 2)
  })

  await t.test('should return 400 for invalid ID format', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    const res = await request(app).delete('/movies/invalid-id')

    assert.equal(res.status, 400)
  })

  await t.test('should call repository delete method', async () => {
    const mockRepo = new MockMovieRepository(sampleMovies)
    const app = createTestApp(mockRepo)

    await request(app).delete('/movies/69263e138d6f9ef025a3be7f')

    const calls = mockRepo.getCallHistory('delete')
    assert.equal(calls.length, 1)
  })
})